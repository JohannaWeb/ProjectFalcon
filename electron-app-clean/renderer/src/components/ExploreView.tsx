      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {selectedUri ? (
          feedPosts.length === 0 ? (
            ...
            <div>
              {feedPosts.map((fp: { post?: { uri?: string }; post: { uri: string; author?: { displayName?: string; handle?: string }; record?: { text?: string } } }, i: number) => (
                <div
                  key={fp.post?.uri ?? i}
                  style={{
                    padding: 12,
                ...
                >
                  ...
                  <p style={{ whiteSpace: 'pre-wrap', marginTop: 4 }}>{(fp.post as { record?: { text?: string } })?.record?.text ?? ''}</p>
                </div>
              ))}
            </div>
          )
