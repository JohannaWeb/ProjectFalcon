...
Falcon is designed with the assumption of an adversarial environment.
\begin{itemize}
    \item \textbf{Sybil Resistance:} The non-linear $tanh$ squashing function ensures that adding thousands of low-quality "trust votes" results in diminishing returns, making large-scale reputation farming economically unviable.
    \item \textbf{Temporal Rot:} Trust is treated as a decaying resource. Without continuous high-integrity interaction, a user's score naturally reverts to the baseline, forcing continuous proof-of-humanity.
\end{itemize}
