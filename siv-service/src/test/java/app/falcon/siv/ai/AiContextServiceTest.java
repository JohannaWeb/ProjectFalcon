...
class AiContextServiceTest {
    ...
    private static final String TAGGING_RESPONSE = "{\"tags\":[\"developer\",\"opensource\"],\"summary\":\"A post about open source development.\",\"factType\":\"TAG\",\"confidence\":0.9,\"reasoning\":\"test logic\"}";
    private static final String SAFE_RESPONSE = "{\"isHarmful\":false,\"reason\":\"\",\"confidence\":0.0,\"reasoning\":\"test logic\"}";
    ...
    void setUp() {
        ...
        when(aiClient.complete(anyString(), anyString()))
                ...
                .thenReturn(Mono.just(SAFE_RESPONSE));
    ...
    void processPostPersistsTagsAndSummary() throws InterruptedException {
        aiContextService.processPost(TEST_DID, "I just shipped my first open source library!", "seq123");
        ...
        // Verify event ID propagation
        assertTrue(captor.getAllValues().stream().allMatch(f -> "seq123".equals(f.getSourceEventId())));
    }

    @Test
    void agentDidIsStampedOnFacts() throws InterruptedException {
        aiContextService.processPost(TEST_DID, "Testing the Falcon AI agent DID stamp.", "seq124");
    ...
    void rateLimitSuppressesSecondCallWithinWindow() throws InterruptedException {
        aiContextService.processPost(TEST_DID, "First post", "seq1");
        Thread.sleep(500);
        reset(aiClient);
        when(aiClient.complete(anyString(), anyString())).thenReturn(Mono.just(TAGGING_RESPONSE));

        aiContextService.processPost(TEST_DID, "Second post within 60s", "seq2");
        Thread.sleep(500);
    ...
    void blankTextIsIgnored() throws InterruptedException {
        aiContextService.processPost(TEST_DID, "   ", "seq_blank");
        aiContextService.processPost(TEST_DID, null, "seq_null");
        Thread.sleep(200);
    ...
    void harmfulContentCreatesWarningFact() throws InterruptedException {
        when(aiClient.complete(anyString(), anyString()))
                ...
                .thenReturn(Mono.just(
                        "{\"isHarmful\":true,\"reason\":\"Explicit harassment.\",\"confidence\":0.92,\"reasoning\":\"harmful test\"}"));

        aiContextService.processPost(TEST_DID, "This is a harmful message.", "seq_harmful");
        Thread.sleep(500);
