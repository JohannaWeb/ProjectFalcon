...
export function Layout({ session, onLogout }: Props) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('juntos:navigate', handler as EventListener)
    return () => window.removeEventListener('juntos:navigate', handler as EventListener)
  }, [])
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          {selectedServerId === 'home' && (
            <>
              {/* Keep Feed and Explore mounted to avoid re-fetching on tab switch */}
              <div style={{ display: view === 'feed' ? 'block' : 'none', height: '100%' }}><FeedView session={sess} /></div>
              <div style={{ display: view === 'explore' ? 'block' : 'none', height: '100%' }}><ExploreView /></div>
              {view === 'dms' && (
